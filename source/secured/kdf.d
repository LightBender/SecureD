...
@trusted public ubyte[] pbkdf2(ubyte[] key, string password, HashFunction func, uint outputLen, uint iterations)
{
    if (key.length == getHashLength(func)) {
        throw new CryptographicException(format("The PBKDF2 key must be %s bytes in length.", getHashLength(func)));
    }
    ...
    version(OpenSSL)
    {
        ...
        if(PKCS5_PBKDF2_HMAC(password.ptr, cast(int)password.length, key.ptr, cast(int)key.length, iterations, getOpenSSLHashFunction(func), outputLen, output.ptr) == 0) {
...
unittest
{
    ...
    ubyte[] vec1 = pbkdf2(key, "", HashFunction.SHA2_384, 48, 25000);
    ubyte[] vec2 = pbkdf2(key, "abc", HashFunction.SHA2_384, 48, 25000);
    ubyte[] vec3 = pbkdf2(key, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", HashFunction.SHA2_384, 48, 25000);
...
unittest
{
    ...
    ubyte[] vec1 = pbkdf2(key, "", HashFunction.SHA2_384, 48, 150000);
    ubyte[] vec2 = pbkdf2(key, "abc", HashFunction.SHA2_384, 48, 150000);
    ubyte[] vec3 = pbkdf2(key, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", HashFunction.SHA2_384, 48, 150000);
...
unittest
{
    ...
    ubyte[] vec1 = pbkdf2(key, "", HashFunction.SHA2_384, 32, 25000);
    ubyte[] vec2 = pbkdf2(key, "abc", HashFunction.SHA2_384, 32, 25000);
    ubyte[] vec3 = pbkdf2(key, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", HashFunction.SHA2_384, 32, 25000);
