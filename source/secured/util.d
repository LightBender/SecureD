...
/*
public uint bytesToUInt(ubyte[4] value) {
    import std.bitmanip;

    return value.read!(uint, Endian.bigEndian);
}

public ulong bytesToULong(ubyte[8] value) {
    import std.bitmanip;

    return value.read!(ulong, Endian.bigEndian);
}

public ubyte[4] uintToBytes(uint value) {
    import std.bitmanip;

    return value.read!(uint, Endian.bigEndian);
}

public ubyte[8] ulongToBytes(ulong value) {
    import std.bitmanip;
